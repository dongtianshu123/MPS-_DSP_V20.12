...
unsigned int step11=0; //�𶯲���
unsigned int step12=0; //���з�����ⲽ��
...
{                  //�ȴ�
   ...
   else if (step11==3) //��
      {
                ...
                StartState.stopflag=0;               
                 ProtectParams.intevalmin=MainParams.SaveParams[18];//���𶯹�Ƶʱ��д���𶯼����������ʱ
 ...
 else if (step11==8)  //ͻ��
   {
        ...
        StartParams.Data =((8000 - MainParams.SaveParams[0])*(100-StartParams.StepVoltage)/100+StartParams.Ugmin);//װ��ͻ����ѹ
	 StartParams.OutData = StartParams.Data >> 10;

           if(StartParams.StartTimeCount>(StartParams.StepDelay*10))//2s��ͨ�ǻص���ʼ��ѹ
             {step11=3;
              StartParams.Data = StartParams.BeginVoltage;
             }
 ...
 else if (step11==11) //���뵼ͨ�Ǽ�С�߼��������߼�1msɨ������   SoftStop1Ms()
   {  step11=12;
   } 
else if (step11==17)
   {    
...
/*******************ͣ���߼�************************************/
 ...
 if (Input.Stop==1 && step11==4 && StartParams.StopTime!=0) //��ͣ
    {           step3=0;
                StartParams.StartTimeCount=0;
                SysStatus = 60;	
		   stopDelayCnt = 0;
...
else if  (Input.Stop==1 &&  StartParams.StopTime==0 && step11 >=2 && step11 <=4) //����ͣ��
    {
            step11=5;   
            OUTPUT_READY = 0;
			OUTPUT_EXTERN_READY = 0;
...
/*************************************************************/
    ...
    if((SysStatus == WAIT)||(SysStatus == READY))    //����ģʽ
    {
...
void EEWrFaultLastTime()
{
	if(MainParams.FaultLastTime != Fault.Byte)
	{
		...
		SET_AND_SAVE_CPU_IPL(savedIPL, 7);	// P0-2:保存原始IPL再提升
		...
		RESTORE_CPU_IPL(savedIPL);			// P0-2:恢复到原始IPL
	}
}
/*
*********************************************************************************************************
