...
void Uart2App()
{
	...
	if(Uart2.TxSetSuccessF)//дE2prom1
	{
		EEWriteApp();
		Uart2TxSetSuccess();
	...
	if(Uart2.TxSetSuccessF2)//дE2prom2
	{
		EEWriteApp2();
		Uart2TxSetSuccess2();
	...
	if(Uart232.TxSetSuccessF3)//дE2prom3
	{
		EEWriteApp3();
		Uart2TxSetSuccess3();
	...
	if(Uart232.TxSetSuccessF4)   //дE2prom4   200421
	{
		EEWriteApp4();
		Uart2TxSetSuccess4();
	...
	if(Uart2.TxReadSetF)//������1
	{
		Uart2TxReadSet();
		Uart2.TxReadSetF = 0;
        U2TxRx.Txcnt=0;
	}
	if(Uart2.TxReadSetF2)//��д����2
	{
		Uart2TxReadSet2();
		Uart2.TxReadSetF2 = 0;
        U2TxRx.Txcnt=0;
	...
	if(Uart232.TxReadSetF4)//��д����4   200421
	{
		Uart2TxReadSet4();
	...
	if(Uart2.TxReadDataF )//������1
	{
		Uart2TxReadData();
		Uart2.TxReadDataF  = 0;
        U2TxRx.Txcnt=0;
	}
	if(Uart2.TxReadDataF2 )//������2
	{
		Uart2TxReadData2();
...
void Uart2TxSetSuccess4()   //������4�ɹ�����  200421
{
	U2TxRx.TxHead = 0XAA;
	U2TxRx.TxOrderType = 0X20;		
...
{	
	...
	SET_AND_SAVE_CPU_IPL(savedIPL, 7);	// P0-2:保存原始IPL
    IEC0bits.ADIE = 0;
	for(i = 0; i < 14; i ++)
    {
	...
	RESTORE_CPU_IPL(savedIPL);		// P0-2:恢复原始IPL
IEC0bits.ADIE = 1;
}	
...
if (Functionswitch.FWD_REV==1)// 0Ϊ��ת 1Ϊ��ת
	...
	if(!Uart2.RxFrameF)
	{
		if (U1STAbits.FERR ==0 )
		{
			...
			else if(U2TxRx.RxTimes == 3)
			{
				...
				if(U2TxRx.RxDataLength == 28)//����1
				{
					U2TxRx.RxTimes = 55;
				}
 	            else if(U2TxRx.RxDataLength == 60)//����2
				{
					U2TxRx.RxTimes = 56;
				}
                else if(U2TxRx.RxDataLength== 1)//����
				{
					U2TxRx.RxTimes = 4;
				}
                else if (U2TxRx.RxDataLength== 0)//������
                {  U2TxRx.RxTimes = 4;
                }
...
void __attribute__((__interrupt__)) _U2TXInterrupt (void)
{
	...
	else if(U2TxRx.TxTimes == 4)
	{
		...
		{
			U2TxRx.TxTimes = 4;
			U2TXREG = U2TxRx.TxData[U2TxRx.TxDataCnt];//������
			U2TxRx.TxDataCnt++;		
			U2TxRx.TxDataLength--;
		}
	}	
...
void Uart2RxApp()
{
	...
	{
		if(U2TxRx.RxOrderID == 0x81)      //�յ�������1
		{
			Uart2.TxReadDataF = 1;
		}
        else if(U2TxRx.RxOrderID == 0x71) //�յ�������2
		{
			Uart2.TxReadDataF2 = 1;
		}

        else if(U2TxRx.RxOrderID == 0x75) //�յ�������2
		{
			Uart2.TxReadDataF3 = 1;
		}
		...
		{
			Uart2.TxReadSetF = 1;         //�յ�������1
		}
		else if(U2TxRx.RxOrderID == 0x72)
		{
			Uart2.TxReadSetF2 = 1;        //�յ�������2
		}
		else if(U2TxRx.RxOrderID == 0x62)
		{
			Uart232.TxReadSetF3 = 1;        //�յ�������3
		}

		else if(U2TxRx.RxOrderID == 0x52)
		{
			Uart232.TxReadSetF4 = 1;        //�յ�������4  200421
		}
	...
	else if(U2TxRx.RxOrderType == 0x20)
	{
		...
		if(U2TxRx.RxOrderID == 0x53)    //�������ɹ�200421
		{
